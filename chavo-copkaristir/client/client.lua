...
CreateThread(function()
    ...
    exports['qb-target']:AddTargetModel(models, {
        options = {
            {
                type = "client",
                event = "chavo-copkaristir:karistir",
...
RegisterNetEvent('chavo-copkaristir:karistir')
AddEventHandler('chavo-copkaristir:karistir', function()
        ...
        if not lastMixedTrash[trashId] or (GetGameTimer() - lastMixedTrash[trashId] >= mixCooldown * 1000) then
            if not aktif then
                ...
                QBCore.Functions.Progressbar("cop_kariştir", "Çöpü Karıştırıyorsun", Config.PickTime, false, true, {
                    ...
                    TriggerServerEvent("chavo-copkaristir:bitir")
